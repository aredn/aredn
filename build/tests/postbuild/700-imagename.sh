...
# Set the MYBUILDNAME var for later
...
if [ "$(find ./bin/* -maxdepth 2 -regex '.*bin/.*[Oo][Pp][Ee][Nn][Ww][Rr][Tt].*' | wc -l)" -eq  "0" ]
then
...
if [ "$(find ./bin/* -maxdepth 2 -regex ".*bin/.*AREDN-$MYBUILDNAME.*" | wc -l)" -eq  "$EXPECTEDFILESCOUNT" ]
then
