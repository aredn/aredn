...
juLog -name="usrlocalbin_luacexists" which luac
LUACNOTEXISTS=$?

# Make sure perl is installed and in path.
juLog -name="usrlocalbin_perlexists" which perl
PERLNOTEXISTS=$?
...
do
  ...
  if echo "$FILE_FIRST_LINE" | grep "/usr/bin/lua" >/dev/null; then
    if [ "$LUACNOTEXISTS" = "1" ]; then
      juLog -name="usrlocalbin_$(basename "$file")" false # Consider test failed if we don't have luac
    else
      juLog -name="usrlocalbin_$(basename "$file")" luac -p "$file"
    fi
  ...
  if echo "$FILE_FIRST_LINE" | grep "bin/sh" >/dev/null; then
    if [ "$SHELLCHECKNOTEXISTS" = "1" ]; then
      ...
      juLog -name="usrlocalbin_$(basename "$file")" shellcheck -e SC2069,SC2009 "$file"
    fi
  continue # Next file please
  fi
  #### End Shell Scripts ####
  ...
  if echo "$FILE_FIRST_LINE" | grep perl >/dev/null; then
    if [ "$PERLNOTEXISTS" = "1" ]; then
      juLog -name="usrlocalbin_$(basename "$file")" false  # Consider test failed if we don't have Perl
    else
      juLog -name="usrlocalbin_$(basename "$file")" "perl -I $AREDNFILESBASE/www/cgi-bin -cw $file"
    fi
  continue # Next file please
  fi
  #### End Perl Scripts ####
